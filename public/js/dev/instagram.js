...
function populateInstragramData(igData,callback,assetLoadLimit=5){
	...
	if(igData['image'].length > 0){
		$('.carousel-inner').append('<div class="carousel-item active"><img alt="Image of Bubbles Thru Hike From Instagram" class="section-instagram-post__img" src="'+igData["image"][0]['url']+'"><input class="instagramCaption" type="hidden" value="'+igData["image"][0]['caption']+'" /></div>');
		
		let imgList = igData['image'].slice(0, assetLoadLimit);
		imgList.forEach(function(value,index){
			if(index!=0){
				$('.carousel-inner').append('<div class="carousel-item"><img alt="Image of Bubbles Thru Hike From Instagram" class="section-instagram-post__img" src="'+value['url']+'"><input class="instagramCaption" type="hidden" value="'+encodeURI(value['caption'].toString())+'" /></div>');
			}
